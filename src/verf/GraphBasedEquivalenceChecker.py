...
class GraphBasedEquivalenceChecker(EquivalenceChecker):
    ...
    def _enumeration_parallel(self, event, sampling_size):
        assert isinstance(self.boolean_function_b, VerilogBenchmark)
        ...
        # Run enumeration
        enumeration = Enumeration(self.boolean_function_a, self.boolean_function_b)
    ...
    def _graph_based_parallel(self, event):
        ...
        for (output_variable, verilog_formula) in self.boolean_function_b.functions.items():
            z3_benchmark_formula = Z3Converter.verilog_to_z3(verilog_formula.verilog)
            ...
            if s.check() == unsat:
                ...
                if config.record_formulae:
                    ...
                    # Get number of literals from formula
    ...
    def _graph_based(self):
        ...
        for (output_variable, verilog_formula) in self.boolean_function_b.functions.items():
            z3_benchmark_formula = z3c.verilog_to_z3(verilog_formula.verilog)
            start_time = time.time()
            z3_crossbar_formula = self.to_formula(output_variable)
            end_time = time.time()
