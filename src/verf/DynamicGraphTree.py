...
class DynamicGraphTree(TreeBasedEquivalenceChecker):
    ...
    def to_formula(self, output_variable: str, input_function: str = "1") -> Bool:
        assert isinstance(self.boolean_function_a, MemristorCrossbar)

        # Construct a bipartite graph where the rows and columns are nodes, and the intersections are edges.
        graph = MultiGraph()
        for layer in range(self.boolean_function_a.layers):
            for r in range(self.boolean_function_a.rows):
                for c in range(self.boolean_function_a.columns):
                    memristor = self.boolean_function_a.get_memristor(r, c, layer)
        ...
        input_layer, input_nanowire = self.boolean_function_a.get_input_nanowire(input_function)
        output_layer, output_nanowire = self.boolean_function_a.get_output_nanowire(output_variable)
        ...
        for l in range(self.boolean_function_a.layers + 1):
            if l % 2 == 0:
                for r in range(self.boolean_function_a.rows):
